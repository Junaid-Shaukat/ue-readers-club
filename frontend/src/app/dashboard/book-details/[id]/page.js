...
const BookDetails = () => {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      try {
        const response = await axios.get(`https://ue-readers-club-backend.vercel.app/admin/getBookById/${id}`);
  ...
  useEffect(() => {
    const trackClick = async () => {
      try {
        if (user && user.id) {
          await axios.put(`https://ue-readers-club-backend.vercel.app/admin/clicks/${id}`, { userId: user.id });
        }
      } catch (err) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        const response = await axios.post('https://ue-readers-club-backend.vercel.app/users/create', userData);
